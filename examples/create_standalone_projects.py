...
def run():
    ...
    projects.append(('catchall', "Catchall",
                     ["navx_frc.json", "REVRobotics.json", "Phoenix.json"]))
    ...
    for package_name, output_project, vendor_props in projects:
        ...
        distutils.dir_util.copy_tree(this_dir + "/project_files/standard",
                                     output_project)

        # Generate the build file
        with open(
                os.path.join(this_dir, "project_files",
                             "build.gradle.template"), 'r') as template_file:
